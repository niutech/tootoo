...
* 2026-09-18 — The branch chip no longer appears as an empty "⎇ ▾" before the tree loads, or on the repo-entry and file-access help screens: a global `[hidden]` rule now makes the `hidden` attribute win over a component's own `display` (the chip's `inline-flex` had been overriding it). Also dropped the stale `favicon*` keys from the `tootoo.config.js` template — core stopped reading them on 2026-07-05
* 2026-09-18 — Fixed checkout/repository/branch mismatches, delayed loads overwriting newer views, branch permalinks and history, keyboard links and folder navigation, query strings and special branch names, prototype-like folder names, and Markdown extension variants. Added [browser regression checks](tootoo-dev/tootoo-test-regressions.html) against the actual assembled app.
* 2026-08-09 — Review pass, five fixes: the branch menu is fully keyboard-operable (Escape closes and refocuses the chip, ArrowUp/Down + Home/End move through options, the current branch is focused on open); the Back button no longer looks dead after the auto-opened README (the first file open of a page load replaces the history entry instead of stacking one); a failed file load can be retried from the same permalink; `.htm` and `.html` share one Rendered/Raw view preference; and selecting a file highlights its tree row from the sidebar's cached row list instead of re-querying the whole tree
